package org.rixel.oldUnusedForReference
{
	...
	public class QuadTree
	{
		...
		{	
			...
			_insertNode = findParentNode(_insertRect,_leafList[this.rixel::getLeafSpacialId(rxDisplayObject.rixel::renderX,rxDisplayObject.rixel::renderY)] as QuadTreeNode);
			
			var proxy:QuadTreeProxy = new QuadTreeProxy(rxDisplayObject,_insertNode);
			
			rxDisplayObject.rixel::proxy = proxy;
			//_insertNode.addDataItem(rxDisplayObject);
		}
		
		public function updateDisplayObject(rxDisplayObject:RxDisplayObject):void
		{
			_insertRect.x = rxDisplayObject.x;
			_insertRect.y = rxDisplayObject.y;
			_insertRect.width = rxDisplayObject.width;
			...
			if(_insertNode != rxDisplayObject.rixel::proxy.node)
			{
				rxDisplayObject.rixel::proxy.remove();
				rxDisplayObject.rixel::proxy.addToNode(_insertNode);	
			}
